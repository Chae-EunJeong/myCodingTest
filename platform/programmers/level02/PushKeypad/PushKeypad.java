...
- 요구사항 : 전화 키패드를 누르는 손이 왼손인지 오른손인지 출력해라
- 함수
    - 입력 : 순서대로 누른 번호 배열 numbers[]
            어느 손 잡이인지 String hand
    - 제약 : 1 <= numbers.length <= 1000
            0 <= numbers[i] <= 9
    - 출력 : numbers를 누른 손들을 띄어쓰기 없이 하나의 문자열로 리턴
- 로직
    2, 5, 8, 0일 때 마지막 왼손가락의 위치와 마지막 오른손가락의 위치의 차가 
            - 같으면 hand에 따라서 결정
            - 차가 더 작은 것의 손으로 설정
*/
